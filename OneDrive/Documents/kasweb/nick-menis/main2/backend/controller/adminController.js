...
export const getPendingPayments = async (req, res) => {
  const payments = await Payments.find()
    ...
    .populate("leadId", "leadName price")
    .sort({ createdAt: -1 });
...
export const approvePayment = async (req, res) => {
  try {
    ...
    // Unlock agent
    const agent = await Users.findByIdAndUpdate(
      payment.agentId,
      {
        hasActiveLead: false,
        activeLeadId: null,
    ...
    if (agent?.email) {
      await sendEmail({
        ...
        html: `
          ...
          <p>Your <strong>1% admin payment</strong> has been approved.</p>
          <p>You may now access new leads.</p>
        `,
      });
    }

    res.json({ message: "Payment approved & agent unlocked" });
  } catch (err) {
